    ...
    (function ($) {
        $(document).ready(function () {
            function updateFields() {
                var traderClsValue = $('#id_strategy_cls').val();
                var csrftoken = $('[name=csrfmiddlewaretoken]').val();
                $.ajax({
                    url: '/admin/workstation/strategyconfig/update_cls/',
                    type: 'POST',
                    data: {'strategy_cls': traderClsValue},
                    headers: {
            ...
            // 监听 trader_cls 字段的变化
            $('#id_strategy_cls').change(updateFields);
        });
    })(django.jQuery);
});
